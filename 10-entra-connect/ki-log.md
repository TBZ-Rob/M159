...
| Fehlerdiagnose Hybrid-Join-Registrierungsfehler | Event-Log-Fehler "The device object by the given id (...) is not found" (Event-ID 304) auf Client01 eingefuegt | Vollstaendigen Event-Log-Eintrag gelesen statt nur Fehlermeldung, `registrationType: sync` erkannt, daraus abgeleitet dass eine neu hinzugefuegte Sync-Regel einen vollstaendigen Initial-Sync statt Delta-Sync braucht | Nach `Start-ADSyncSyncCycle -PolicyType Initial` und erneutem Task-Lauf: `AzureAdJoined: YES`. Auftrag 10 damit inhaltlich abgeschlossen |
